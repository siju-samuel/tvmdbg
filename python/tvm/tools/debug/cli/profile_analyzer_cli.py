# coding: utf-8
# pylint: disable=fixme, too-many-arguments, too-many-locals, too-many-statements, too-many-return-statements
...
class ProfileDataTableView(object):
    ...
    def row_count(self):
        """Get the size of row cell of the table.

        Returns:
          ('int') The size of row cell of the table.
        """
        return len(self._profile_datum_list)

    def column_count(self):
        """Get the size of column cell of the table.

        Returns:
          ('int') The size of column cell of the table.
        """
        return len(self._column_names)

    def column_names(self):
        """Get the cell column names of the table.

        Returns:
          ('list) List of cell column names of the table.
        """
        return self._column_names

    def column_sort_id(self, col):
        """Get the cell column sort name of the table.

        Args:
          col: (int) column index.

        Returns:
          ('string') Table column sort name.
        """
...
class ProfileAnalyzer(object):
    ...
    def __init__(self, graph, run_metadata):
        ...
        Args:
          graph: (tvm.Graph) Python graph object.
        ...
        argument_parser.add_argument(
            ...
            help="filter by file name at the top position of node's creation "
                 "stack that does not belong to TVM library.")
        argument_parser.add_argument(
            "--min_lineno",
            dest="min_lineno",
            type=int,
        ...
        self._arg_parsers["list_profile"] = argument_parser
        ...
        self._arg_parsers["print_source"] = argument_parser
    ...
    def _get_profile_data_generator(self):
        ...
        node_to_func_name = {}
        node_to_op_type = {}
        for operation in self._graph.get_operations():
            for trace_entry in reversed(operation.traceback):
                file_path = trace_entry[0]
                line_num = trace_entry[1]
                func_name = trace_entry[2]
                if not source_utils.guess_is_tvm_py_library(file_path):
                    break
            node_to_file_path[operation.name] = file_path
            node_to_line_number[operation.name] = line_num
            node_to_func_name[operation.name] = func_name
            node_to_op_type[operation.name] = operation.type
        ...
        for i, line in enumerate(source_lines):
            ...
            if lineno in source_annotation:
                ...
                cost_bar = self._render_normalized_cost_bar(
                    _get_total_cost(annotation, parsed.cost_type), max_total_cost,
                    cost_bar_max_length)
...
def create_profiler_ui(graph,
                       ...
                       on_ui_exit=None,
                       config=None):
    """Create an instance of CursesUI based on a `tvm.Graph` and `RunMetadata`.
...
def _get_list_profile_lines(
        device_name, device_index, device_count,
...
def _get_total_cost(aggregated_profile, cost_type):
