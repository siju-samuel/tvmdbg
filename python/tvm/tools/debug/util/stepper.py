# coding: utf-8
# pylint: disable=fixme, too-many-branches, too-many-statements, too-many-locals, too-many-arguments, too-many-instance-attributes, invalid-name
"""TVM Debugger (tvmdbg) Stepper Module."""
...
class NodeStepper(object):
    ...
    def _dfs_visit(self, graph, elem_list):
        ...
        closure_elements = [elem for elem in elem_list]

        ref_tensor_names = set()
        for element in elem_list:
            # """isinstance(element, ops.Tensor)"""
            if False and element.dtype._is_ref_dtype:  # pylint: disable=protected-access
                # ref_tensor_names.add(element.name)
                ref_tensor_names.add(element)
        ...
        while elem_stack:
            ...
            for inp in all_inputs:
                ...
                # """isinstance(inp, ops.Tensor)"""
                if False and inp.dtype._is_ref_dtype:  # pylint: disable=protected-access
    ...
    def override_tensor(self, tensor_name, overriding_val):
        ...
        node_name = _get_node_name(tensor_name)
...
#              self._tensor_handles[handle_name] = handle
    ...
    def _prepare_cont_call_dump_path_and_run_options(self):
        ...
        """
        run_options = None  # config_pb2.RunOptions()
    ...
    def _load_dumped_intermediate_tensors(self, dump_path, target_name):
        dump_dir = debug_data.DebugDumpDir(self._ctx, dump_path, validate=False)
    ...
    def handle_node_names(self):
        ...
        return set([_get_node_name(name) for name in self._tensor_handles])
    ...
    def is_placeholder(self, graph_element_name):
        ...
        node_name = _get_node_name(graph_element_name)
