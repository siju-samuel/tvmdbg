...
class ProfileDatum(object):# pylint: disable=too-few-public-methods
    """Profile data point."""
...
class AggregateProfile(object):
    ...
    @property
    def node_count(self):
        """Get size of node count to be execute.

        Returns:
          ('int') size of node to execute.
        """
        return len(self._node_to_exec_count)

    @property
    def node_exec_count(self):
        """Get sum of all node count to be execute.

        Returns:
          ('int') size of node to execute values.
        """
