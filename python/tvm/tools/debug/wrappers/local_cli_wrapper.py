# coding: utf-8
# pylint: disable=fixme, too-many-instance-attributes, invalid-name, too-many-arguments, abstract-method
...
#from tvm.tools.debug.cli import profile_analyzer_cli
...
class LocalCLIDebugWrapperModule(framework.BaseDebugWrapperModule):
    ...
    def _initialize_argparsers(self):
        ...
        ap.add_argument(
            ...
            dest="profile",
            action="store_true",
            help="Run and profile Tvm graph execution.")
    ...
    def on_run_end(self, request):
        ...
        if request.performed_action == common.CLIRunStartAction.DEBUG_RUN:
            ...
            #debug_dump.set_python_graph(self.graph)
            ...
            self._remove_dump_root()
        # elif request.performed_action == common.CLIRunStartAction.PROFILE_RUN:
        ...
        if parsed.profile:
            raise debugger_cli_common.CommandLineExit(
                exit_token=framework.OnRunStartResponse(
                    common.CLIRunStartAction.PROFILE_RUN, []))
...
#            prefix_aliases=["s"])
    ...
    def _on_run_start_step_handler(self, args, screen_info=None):
        ...
        raise debugger_cli_common.CommandLineExit(
            exit_token=framework.OnRunStartResponse(
                common.CLIRunStartAction.INVOKE_STEPPER, []))
